...
const mferFile = path.join(__dirname, 'modules/mfer-meme.js');
let code = fs.readFileSync(mferFile, 'utf8');

const tplOrbLandscape = `
  // orb-landscape (animated Aurora signature)
  'orb-landscape': {
    description: 'Aurora signature animated orb over dark landscape, mfer standing below.',
    fields: ['top', 'bottom'],
    examples: [{ top: 'what I make', bottom: 'what I remember making' }],
    svgRenderer: (texts, mferTraits) => {
      const W = 500, H = 420;
      const mferId = randomMferId();
      const mfer = mferTraits || getMferTraits(mferId);
      const palettes = [
        { sky1:'#050510', sky2:'#0d1535', sky3:'#060818', orb1:'#ffffff', orb2:'#88aaff', orb3:'#2244bb', mtn:'#0d0d22', water:'#06070f' },
        { sky1:'#0f080a', sky2:'#1a0c10', sky3:'#070408', orb1:'#fff4e0', orb2:'#ffaa44', orb3:'#bb4400', mtn:'#120808', water:'#060304' },
        { sky1:'#05100a', sky2:'#0a1e12', sky3:'#030806', orb1:'#e0fff5', orb2:'#44ffcc', orb3:'#006644', mtn:'#060e08', water:'#030605' },
        { sky1:'#0d0518', sky2:'#180d2a', sky3:'#080510', orb1:'#f0e0ff', orb2:'#cc88ff', orb3:'#660099', mtn:'#100818', water:'#060408' },
      ];
      const pal = palettes[Math.floor(Math.random() * palettes.length)];
      const animated = Math.random() < 0.75;
      const stars = Array.from({length: 36}, (_, i) => {
        const x = ((i * 137 + 23) % 470) + 15;
        const y = ((i * 79 + 11) % 190) + 8;
        const r = i % 5 === 0 ? 1.5 : 0.7;
        const op = (0.3 + ((i * 11) % 7) * 0.1).toFixed(2);
        const tw = animated && i % 4 === 0 ? '<animate attributeName="opacity" values="' + op + ';' + (parseFloat(op)*0.3).toFixed(2) + ';' + op + '" dur="' + (3 + (i%5)) + 's" repeatCount="indefinite"/>' : '';
        return '<circle cx="' + x + '" cy="' + y + '" r="' + r + '" fill="white" opacity="' + op + '">' + tw + '</circle>';
      }).join('');
      const shimmer = Array.from({length: 7}, (_, i) => '<line x1="' + (130+i*28) + '" y1="' + (325+(i%2)*5) + '" x2="' + (165+i*28) + '" y2="' + (325+(i%2)*5) + '" stroke="' + pal.orb2 + '" stroke-width="0.8" opacity="0.18"/>').join('');
      const animR = animated ? '<animate attributeName="r" values="53;62;53" dur="5s" repeatCount="indefinite"/>' : '';
      const animOp = animated ? '<animate attributeName="opacity" values="0.07;0.14;0.07" dur="5s" repeatCount="indefinite"/>' : '';
      const animRef = animated ? '<animate attributeName="opacity" values="0.08;0.18;0.08" dur="5s" repeatCount="indefinite"/>' : '';
      let svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 420" width="500" height="420">'
        + '<defs><linearGradient id="lgs" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="' + pal.sky1 + '"/><stop offset="55%" stop-color="' + pal.sky2 + '"/><stop offset="100%" stop-color="' + pal.sky3 + '"/></linearGradient>'
        + '<radialGradient id="rgo" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="' + pal.orb1 + '" stop-opacity="0.96"/><stop offset="30%" stop-color="' + pal.orb2 + '" stop-opacity="0.72"/><stop offset="65%" stop-color="' + pal.orb3 + '" stop-opacity="0.32"/><stop offset="100%" stop-color="' + pal.orb3 + '" stop-opacity="0"/></radialGradient></defs>'
        + '<rect width="500" height="420" fill="url(#lgs)"/>' + stars
        + '<circle cx="250" cy="150" r="95" fill="' + pal.orb3 + '" opacity="0.07"/>'
        + '<circle cx="250" cy="150" r="75" fill="' + pal.orb2 + '" opacity="0.09"/>'
        + '<circle cx="250" cy="150" r="56" fill="url(#rgo)">' + animR + '</circle>'
        + '<circle cx="250" cy="150" r="44" fill="' + pal.orb1 + '" opacity="0.09">' + animOp + '</circle>'
        + '<circle cx="237" cy="137" r="12" fill="white" opacity="0.18"/>'
        + '<path d="M0 295 L60 235 L115 268 L168 208 L225 252 L278 188 L332 238 L388 212 L442 250 L500 222 L500 295 Z" fill="' + pal.mtn + '"/>'
        + '<path d="M0 312 L45 272 L92 293 L145 258 L198 280 L252 250 L306 274 L358 260 L412 278 L465 262 L500 272 L500 312 Z" fill="' + pal.mtn + '" opacity="0.65"/>'
        + '<rect x="0" y="312" width="500" height="108" fill="' + pal.water + '"/>'
        + '<ellipse cx="250" cy="358" rx="44" ry="11" fill="' + pal.orb2 + '" opacity="0.12">' + animRef + '</ellipse>'
        + '<ellipse cx="250" cy="350" rx="26" ry="5" fill="' + pal.orb1 + '" opacity="0.09"/>' + shimmer;
      svg += drawMfer(mfer, 250, 280, 0.62, 'standing', true);
      const top = (texts.top || '').substring(0, 30);
      const bot = (texts.bottom || '').substring(0, 30);
      if (top) svg += '<text x="250" y="30" text-anchor="middle" font-size="17" font-weight="900" fill="#e8f4ff" font-family="Arial Black,sans-serif" stroke="#000" stroke-width="3" paint-order="stroke">' + top + '</text>';
      if (bot) svg += '<text x="250" y="' + (H-12) + '" text-anchor="middle" font-size="17" font-weight="900" fill="#ffffff" font-family="Arial Black,sans-serif" stroke="#000" stroke-width="3" paint-order="stroke">' + bot + '</text>';
      svg += '<text x="' + (W-8) + '" y="' + (H-3) + '" text-anchor="end" font-size="10" fill="#ffffff18">mfer #' + mferId + '</text></svg>';
      return { svg, mferId };
    }
  },
`;
...
const tplTwoOrbs = `
  ...
  'two-orbs': {
    description: 'Two glowing orbs over landscape, two mfers below. Left=bright blue, right=dim purple. For contrast memes.',
    fields: ['left', 'right'],
    ...
    svgRenderer: (texts) => {
      ...
      const animR2 = animated ? '<animate attributeName="opacity" values="0.8;1;0.8" dur="6s" repeatCount="indefinite"/>' : '';
      const animRef2 = animated ? '<animate attributeName="opacity" values="0.07;0.15;0.07" dur="4.5s" repeatCount="indefinite"/>' : '';
      let svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 560 420" width="560" height="420">'
        ...
        + '<circle cx="402" cy="148" r="34" fill="url(#tobR)">' + animR2 + '</circle>'
        ...
        + '<ellipse cx="158" cy="352" rx="30" ry="8" fill="#88bbff" opacity="0.1">' + animRef2 + '</ellipse>'
...
const tplOrbMeme = `
  ...
  'orb-meme': {
    description: 'Pure Aurora orb in cosmic void, mountain landscape, twinkling stars. No mfer. For philosophical/poetic captions.',
    fields: ['top', 'bottom'],
...
if (oldOrbRx.test(code)) {
  ...
  console.log('⚠️  orb-landscape not found — will append');
}

// Insert two-orbs + orb-meme before closing of TEMPLATES
...
if (insertIdx !== -1) {
  ...
  console.log('⚠️  Could not find insertion point'); process.exit(1);
}

fs.writeFileSync(mferFile, code);
console.log('\n✅ Done! 3 orb templates patched in.');

// Fix insertion point
const fs2 = require('fs');
const path2 = require('path');
const mferFile2 = path2.join(__dirname, 'modules/mfer-meme.js');
let code2 = fs2.readFileSync(mferFile2, 'utf8');

const insertIdx2 = code2.lastIndexOf('\n};\n\n// ===');
if (insertIdx2 !== -1) {
  code2 = code2.slice(0, insertIdx2) + '\n' + tplTwoOrbs + '\n' + tplOrbMeme + code2.slice(insertIdx2);
  fs2.writeFileSync(mferFile2, code2);
  console.log('✅ two-orbs and orb-meme added!');
} else {
  console.log('still not found — run: grep -n "^};" ~/Desktop/aurora-agent/modules/mfer-meme.js');
}
