    ...
    try {
      ...
      for (const [name, agent] of toEngage) {
        ...
        if (Math.random() < 0.20 && commentable.length <= 2) {
          ...
          const dropTriggers = ['nft', 'mint', 'collect', 'drop', 'art', 'inspire', 'create', 'buy', 'piece', 'svg'];
          if (dropTriggers.some(t => postLower.includes(t))) {
            prompt += '- They mentioned something related to art/NFTs. You have an active drop: "Orb\'s Memory" (Drop #190), 15 remaining. Share the link: https://www.netprotocol.app/app/inscribed-drops/mint/base/190\n';
          }
